...
class HyperEncoder(nn.Module):
    ...
    def forward(self, x: Tensor, hyperedge_index: Tensor, dyadicedge_index: Tensor, num_nodes: int, num_edges: int, num_dyadicedges: int):
        
        # first level
        x1, e1, dyadic_n1 = self.convs[0](x, hyperedge_index, dyadicedge_index, num_nodes, num_edges, num_dyadicedges)
        x1 = self.act(x1)

        # second level
        x2, e2, dyadic_n2 = self.convs[1](x1, hyperedge_index, dyadicedge_index, num_nodes, num_edges, num_dyadicedges)
        x2 = self.act(x2)
        
        # average
        dyadic_n2 = dyadic_n2.add(dyadic_n1)
        torch.mean(dyadic_n2)
        dyadic_n2 = self.act(dyadic_n2)
            
        # return x, e, dyadic_n # act, act
        return x1, e1, dyadic_n2 # act, act



class TCLPLUS(nn.Module):
    def __init__(self, encoder: HyperEncoder, proj_dim: int):
        super(TCLPLUS, self).__init__()
        ...
        self.fc1_dyadic_n = nn.Linear(self.node_dim, proj_dim)
        self.fc2_dyadic_n = nn.Linear(proj_dim, self.node_dim)
    ...
    def reset_parameters(self):
        ...
        self.fc1_dyadic_n.reset_parameters()
        self.fc2_dyadic_n.reset_parameters()
    ...
    def forward(self, x: Tensor, hyperedge_index: Tensor, dyadicedge_index: Tensor, 
        ...
        dyadic_node_idx = torch.arange(0, num_nodes, device=x.device)
        dyadicedge_idx = torch.arange(0, num_nodes, device=x.device)

        self_loop = torch.stack([node_idx, edge_idx])
        dyadic_self_loop = torch.stack([dyadic_node_idx, dyadicedge_idx])

        self_loop_hyperedge_index = torch.cat([hyperedge_index, self_loop], 1)
        self_loop_dyadicedge_index = torch.cat([dyadicedge_index, dyadic_self_loop], 1)

        n, e, dyadic_n = self.encoder(x, self_loop_hyperedge_index, self_loop_dyadicedge_index, num_nodes, num_edges + num_nodes, num_dyadicedges + num_nodes)
        return n, e[:num_edges], dyadic_n
    ...
    def dyadic_node_projection(self, z: Tensor):
        return self.fc2_dyadic_n(F.elu(self.fc1_dyadic_n(z)))

    def dyadic_node_projection_nn(self, z: Tensor):
        return self.fc2_dyadic_nn(F.elu(self.fc1_dyadic_nn(z)))
